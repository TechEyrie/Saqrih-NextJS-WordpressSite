"use client";

import Link from "next/link";
import { useEffect, useState } from "react";
import { ChevronLeft, Mail } from "lucide-react";

const GREEN = "#162D24";
const BIO_BG = "#F3E8DC";
const FONT_HEADING = "var(--font-inter), Inter, Arial, sans-serif";
const FONT_BODY = "var(--font-montserrat), Montserrat, Arial, sans-serif";

function ShareIcon({ label, children, href }) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={`Share via ${label}`}
      className="blog-share-icon"
      style={{
        width: 36,
        height: 36,
        borderRadius: "50%",
        border: "1px solid rgba(22, 45, 36, 0.2)",
        display: "inline-flex",
        alignItems: "center",
        justifyContent: "center",
        color: GREEN,
        textDecoration: "none",
        transition: "background-color 0.2s ease, border-color 0.2s ease",
      }}
    >
      {children}
    </a>
  );
}

/**
 * @param {{ author: object; previousPost?: object }} props
 */
export default function BlogPostFooter({ author, previousPost }) {
  const [shareUrl, setShareUrl] = useState("");

  useEffect(() => {
    setShareUrl(window.location.href);
  }, []);

  const encoded = encodeURIComponent(shareUrl);

  return (
    <footer className="blog-post-footer">
      <div
        style={{
          marginTop: "clamp(40px, 4.5vw, 56px)",
          display: "flex",
          flexWrap: "wrap",
          alignItems: "center",
          gap: "clamp(10px, 1.4vw, 14px)",
        }}
      >
        <span
          style={{
            color: GREEN,
            fontFamily: FONT_BODY,
            fontSize: "0.92rem",
            fontWeight: 600,
          }}
        >
          Share via:
        </span>
        <ShareIcon label="Email" href={`mailto:?body=${encoded}`}>
          <Mail size={16} strokeWidth={1.75} />
        </ShareIcon>
        <ShareIcon label="Facebook" href={`https://www.facebook.com/sharer/sharer.php?u=${encoded}`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
            <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.41c0-3.025 1.792-4.697 4.533-4.697 1.312 0 2.686.236 2.686.236v2.971h-1.513c-1.491 0-1.956.93-1.956 1.886v2.267h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z" />
          </svg>
        </ShareIcon>
        <ShareIcon label="X" href={`https://twitter.com/intent/tweet?url=${encoded}`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
          </svg>
        </ShareIcon>
        <ShareIcon label="LinkedIn" href={`https://www.linkedin.com/sharing/share-offsite/?url=${encoded}`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
          </svg>
        </ShareIcon>
        <ShareIcon label="Pinterest" href={`https://pinterest.com/pin/create/button/?url=${encoded}`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
            <path d="M12 0C5.373 0 0 5.372 0 12c0 5.084 3.163 9.426 7.627 11.174-.105-.949-.2-2.403.042-3.441.218-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738a.36.36 0 01.083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.631-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0z" />
          </svg>
        </ShareIcon>
        <ShareIcon label="Reddit" href={`https://www.reddit.com/submit?url=${encoded}`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
            <path d="M12 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 11-2.498-.064c0-.687.561-1.185 1.248-1.185zm-9.999 3.28a3.287 3.287 0 00-3.284 3.282 3.287 3.287 0 003.284 3.282 3.287 3.287 0 003.284-3.282 3.287 3.287 0 00-3.284-3.282zm9.999 0a3.287 3.287 0 00-3.284 3.282 3.287 3.287 0 003.284 3.282 3.287 3.287 0 003.284-3.282 3.287 3.287 0 00-3.284-3.282z" />
          </svg>
        </ShareIcon>
      </div>

      <div
        className="blog-author-bio"
        style={{
          marginTop: "clamp(28px, 3.2vw, 40px)",
          padding: "clamp(24px, 3vw, 32px)",
          borderRadius: "clamp(16px, 2vw, 20px)",
          backgroundColor: BIO_BG,
          display: "flex",
          gap: "clamp(18px, 2.4vw, 28px)",
          alignItems: "flex-start",
        }}
      >
        <img
          src={author.avatar}
          alt=""
          width={88}
          height={88}
          style={{
            width: "clamp(72px, 8vw, 88px)",
            height: "clamp(72px, 8vw, 88px)",
            borderRadius: "50%",
            objectFit: "cover",
            flexShrink: 0,
          }}
        />
        <div>
          <p
            style={{
              margin: 0,
              color: GREEN,
              fontFamily: FONT_HEADING,
              fontSize: "clamp(1.05rem, 1.2vw, 1.15rem)",
              fontWeight: 700,
            }}
          >
            {author.name}
          </p>
          {author.url ? (
            <a
              href={author.url}
              target="_blank"
              rel="noopener noreferrer"
              style={{
                display: "inline-block",
                marginTop: 4,
                color: GREEN,
                fontFamily: FONT_BODY,
                fontSize: "0.88rem",
                opacity: 0.75,
                textDecoration: "underline",
              }}
            >
              {author.url.replace(/^https?:\/\//, "")}
            </a>
          ) : null}
          <p
            style={{
              margin: "clamp(12px, 1.4vw, 16px) 0 0",
              color: "#2a2a2a",
              fontFamily: FONT_BODY,
              fontSize: "clamp(0.95rem, 1.02vw, 1rem)",
              lineHeight: 1.7,
            }}
          >
            {author.bio}
          </p>
        </div>
      </div>

      {previousPost ? (
        <Link
          href={previousPost.href}
          className="blog-prev-post"
          style={{
            display: "inline-flex",
            flexDirection: "column",
            gap: 6,
            marginTop: "clamp(32px, 3.6vw, 44px)",
            textDecoration: "none",
            color: GREEN,
            maxWidth: "min(520px, 100%)",
          }}
        >
          <span
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: 6,
              fontFamily: FONT_BODY,
              fontSize: "0.82rem",
              fontWeight: 600,
              letterSpacing: "0.06em",
              textTransform: "uppercase",
              opacity: 0.7,
            }}
          >
            <ChevronLeft size={16} strokeWidth={2} aria-hidden />
            Previous Post
          </span>
          <span
            style={{
              fontFamily: FONT_HEADING,
              fontSize: "clamp(1rem, 1.15vw, 1.12rem)",
              fontWeight: 700,
              lineHeight: 1.35,
              textDecoration: "underline",
              textUnderlineOffset: 3,
            }}
          >
            {previousPost.title}
          </span>
        </Link>
      ) : null}

      <style>{`
        .blog-share-icon:hover {
          background-color: rgba(22, 45, 36, 0.08);
          border-color: rgba(22, 45, 36, 0.35);
        }
        .blog-prev-post:hover span:last-child {
          opacity: 0.85;
        }
        @media (max-width: 560px) {
          .blog-author-bio {
            flex-direction: column;
            align-items: center;
            text-align: center;
          }
        }
      `}</style>
    </footer>
  );
}
