...
import { getCaseStudyCarouselCards } from "../../lib/caseStudyCarousel";

gsap.registerPlugin(SplitText, ScrollTrigger);

function buildCards(pageKey) {
  const pics = getPageSectionPics(pageKey, "endToEnd");
  return getCaseStudyCarouselCards("home1").map((card, i) => ({
    ...card,
    img: card.img ?? pics[i] ?? pics[0],
  }));
}

export default function EndToEndSection({ theme = "dark", pageKey = "home1" }) {
